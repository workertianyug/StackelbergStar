...
class SentdeBot(sc2.BotAI):
    ...
    async def on_step(self, iteration):
        ...
        self.mytime = (self.state.game_loop/22.4) / 60
    ...
    async def build_scout(self):
        if len(self.units(OBSERVER)) < math.floor(self.mytime/3):
            for rf in self.units(ROBOTICSFACILITY).ready.idle:
                print(len(self.units(OBSERVER)), self.mytime/3)
    ...
    async def expand(self):
        try:
            if self.units(NEXUS).amount < self.mytime/2 and self.can_afford(NEXUS):
    ...
    async def offensive_force_buildings(self):
        if self.units(PYLON).ready.exists:
            ...
            if self.units(CYBERNETICSCORE).ready.exists:
                if len(self.units(STARGATE)) < self.mytime:
    ...
    async def attack(self):
        ...
        if len(self.units(VOIDRAY).idle) > 0:
            ...
            target = False
            if self.mytime > self.do_something_after:
                if self.use_model:
                ...
                if choice == 0:
                    ...
                    self.do_something_after = self.mytime + wait
